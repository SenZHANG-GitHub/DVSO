...
MinimalImageB16* readImageBW_16U(std::string filename)
{
...
{
	// cv::Mat(numBytes, 1, CV_8U, data) is a numBytes x 1 Mat
	// How come m becomes a m.rows x m.cols matrix? 
	// cv::imdecode will determine the shape from stream data as cv::imread do
...
{
	// cv::Mat(numBytes, 1, CV_16U, data) is a numBytes x 1 Mat
	// How come m becomes a m.rows x m.cols matrix? 
	// cv::imdecode will determine the shape from stream data as cv::imread do
	cv::Mat m = cv::imdecode(cv::Mat(numBytes,1,CV_16U, data), CV_LOAD_IMAGE_UNCHANGED);
	if(m.rows*m.cols==0)
	{
		printf("cv::imdecode could not read stream (%d bytes)! this may segfault. \n", numBytes);
		return 0;
