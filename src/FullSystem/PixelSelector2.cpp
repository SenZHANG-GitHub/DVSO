...
//	else
	...
	if(quotia < 0.95) // Sen -> quotia = numWant / numHave
	{
		int wh=wG[0]*hG[0];
		int rn=0;
		...
		for(int i=0;i<wh;i++)
		{
			if(map_out[i] != 0)
			{
				// Sen -> use proportion as probability to delete redundant points
		...
		float* map_out, int pot, float thFactor)
{
	...
	// Sen -> setting_gradDownweightPerLevel = 0.75
	float dw1 = setting_gradDownweightPerLevel; 
	float dw2 = dw1*dw1;


	// Sen -> pot: currentPotential = 3
	// Sen -> include points with weaker gradients 
	// Sen -> thus repeat twice with decreased gradient threshold and block-size 2d and 4d
	int n3=0, n2=0, n4=0;
			...
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot)
			{
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1)
				{
					...
					if(ag0 > pixelTH0*thFactor) 
					{
						...
						// Sen -> setting_selectDirectionDistribution = true
