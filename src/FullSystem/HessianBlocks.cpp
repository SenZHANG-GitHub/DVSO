...
bool PointHessian::isWarpValid()
{
	Vec3f pDisp_left = (getInterpolatedElement33(host->dDisp, u, v, wG[0]));
	Vec3f pDisp_right = (getInterpolatedElement33(host->frame_right->dDisp, u - pDisp_left[0], v, wG[0]));
	if (pDisp_right[0] < 1e-6)
		return false;
	return true;
}
...
void FrameHessian::makeImages(float* color, CalibHessian* HCalib)
{
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		if(lvl>0)
		{
			...
			// Sen -> average of the four corners in last pyramid lvl
			for(int y=0;y<hl;y++)
				for(int x=0;x<wl;x++)
				{
		...
		{
			// Sen -> calculate the image gradient numerically
...
void FrameHessian::makeDisps(float* disp, bool is_warped)
{
    // Sen -> We don't need HCalib here since disparities are not calibrated while images are calibrated instead
	// Sen -> disp unit: m; dDisp[i][0] unit: pixel!!!
	int w=wG[0];
	int h=hG[0];

	dDisp = new Eigen::Vector3f[w*h];

	// make d0
	
	for(int i=0;i<w*h;i++)
	{
		// // Sen -> 0 means missing holes in warped disparities
		// if(is_warped && disp[i] < 1e-6)
		// 	dDisp[i][0] = 0.;
		// else

		// Sen -> Though from monodepth(2)'s implementation disp is always non-negative
		assert(disp[i] >= 0);
		dDisp[i][0] = disp[i] * w;
	}
    ...
    for(int idx=w; idx < w*(h-1); idx++)
    {
		...
		if (maskWarpGrad && is_warped)
		{
			// Sen -> Set the gradient of warped holes to be 0
			if (dDisp[idx+1][0] > 1e-6 && dDisp[idx-1][0] > 1e-6)
				dx = 0.5f*(dDisp[idx+1][0] - dDisp[idx-1][0]);
			if (dDisp[idx+w][0] > 1e-6 && dDisp[idx-w][0] > 1e-6)	
				dy = 0.5f*(dDisp[idx+w][0] - dDisp[idx-w][0]);
		} else 
		{
			dx = 0.5f*(dDisp[idx+1][0] - dDisp[idx-1][0]);
			dy = 0.5f*(dDisp[idx+w][0] - dDisp[idx-w][0]);
	...
	// }
	// bool stop=true;
	// printf("=> iswarped: %d -> min: %.2f\n", is_warped, min);
	// printf("=> iswarped: %d -> max: %.2f\n", is_warped, max);
}
...
void FrameFramePrecalc::set(FrameHessian* host, FrameHessian* target, CalibHessian* HCalib )
{
	...
	// Sen -> the distanceLL between frames is determined by translation norm
