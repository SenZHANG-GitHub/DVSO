...
ImmaturePointStatus ImmaturePoint::traceOn(FrameHessian* frame, const Mat33f &hostToFrame_KRKi, const Vec3f &hostToFrame_Kt, const Vec2f& hostToFrame_affine, CalibHessian* HCalib, bool debugPrint)
{
	...
	float maxPixSearch = (wG[0]+hG[0])*setting_maxPixSearch; // maximum length for epipolar line search
...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
	...
	{
		// Sen -> idepth_max is initialized as NAN 
		dist = maxPixSearch;
		...
		float idepth)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		// Sen -> determine whether it is within the boundary after projecting to current frame
...
// // SenFlagVSImmature
// double ImmaturePoint::linearizeVirtualStereoResidual(
// 		CalibHessian *  HCalib, const float outlierTHSlack,
// 		ImmaturePointTemporaryResidual* tmpRes,
// 		float &Hdd, float &bd,
// 		float idepth)
...
// 	float energyLeft=0;
// 	const Eigen::Vector3f* dIl_left = tmpRes->host->dI;
// 	const Eigen::Vector3f* dDisp_right = tmpRes->target->dDisp;

// 	Mat33f PRE_RTll = Mat33f::Identity();
// 	Vec3f PRE_tTll = Vec3f::Zero();
// 	PRE_tTll << -baseline, 0, 0;
...
// 		if(!std::isfinite((float)hitDisp_right[0]) || !std::isfinite((float)hitColor_left[0])) 
// 		{
...
// 		// depth derivatives.
// 		float dxInterp = (1 - hitDisp_right[1]) * hitColor_left[1] * HCalib->fxl();
// 		float dyInterp = ( -hitDisp_right[2] * hitColor_left[1] + hitColor_left[2] ) * HCalib->fyl();
// 		float d_idepth = derive_idepth(PRE_tTll, u, v, dx, dy, dxInterp, dyInterp, drescale);

// 		hw *= weights[idx]*weights[idx];

// 		// Sen -> If we use w_energyStereo here, it will not be counted for outlier check
// 		hw *= w_energyStereo;
