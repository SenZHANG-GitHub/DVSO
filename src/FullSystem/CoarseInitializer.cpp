...
CoarseInitializer::~CoarseInitializer()
{
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		if(idepth[lvl] != 0) delete[] idepth[lvl];
	}

	delete[] JbBuffer;
	delete[] JbBuffer_new;
	delete[] firstDispLeft;
}


// Sen -> trackFrame()
//       (1) Optimize idepth in each lvl in the pyramid 
//       (2) Propagate between lvls
bool CoarseInitializer::trackFrame(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	newFrame = newFrameHessian;

    // * Step 1 -> Display the new frame before tracking
    ...
    // * Step 2 -> (1) Initialize the idepth to 1 for each point (2) Initialize photometric parameters and SE3 pose
    //          -> snapped: whether the optimization for current frame is satisfactory or not
	if(!snapped) 
	{
	...
	// Sen -> (1) Initialize the affine parameters (2) ab_exposure = image->exposure_time
	if(firstFrame->ab_exposure>0 && newFrame->ab_exposure>0)
		refToNew_aff_current = AffLight(logf(newFrame->ab_exposure /  firstFrame->ab_exposure),0); // coarse approximation.


	// Estimate starting from the top lvl
	Vec3f latestRes = Vec3f::Zero();
	for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
	{

        // * Step 3 -> Use the computed results from last lvl to initilize current lvl
		//          -> propagateDown(): propagate point-related parameters，i.e., iR, idepth, idepth_new: all set to newiR
        ...
        // * Step 4 -> Calculate the energy and Hessian
		Vec3f resOld = calcResAndGS(lvl, H, b, Hsc, bsc, refToNew_current, refToNew_aff_current, false);
		applyStep(lvl); // Apply the new energy to replace the old one
		float lambda = 0.1;
		float eps = 1e-4;
		int fails=0;
		...
		if(printDebug)
		{
			printf("lvl %d, it %d (l=%f) %s: %.3f+%.5f -> %.3f+%.5f (%.3f->%.3f) (|inc| = %f)! \t",
					lvl, 0, lambda,
					"INITIA",
					sqrtf((float)(resOld[0] / resOld[2])), // res x res average
					sqrtf((float)(resOld[1] / resOld[2])), // EAlpha average of the energy of idepth
		...
		// * Step 5 -> Iteratively solve the optimization problem
		int iteration=0;
		while(true)
		{
            // * Step 5.1 -> Compute the marginalized Hessian
			Mat88f Hl = H;
			// Sen -> Levenberg-Marquardt
			for(int i=0;i<8;i++) Hl(i,i) *= (1+lambda); 
			// Schur Complement for idepth marginalization
			Hl -= Hsc*(1/(1+lambda)); 
			...
			// * Step 5.2 -> Compute the increment
			Vec8f inc;
			if(fixAffine) 
			{
			...
			// * Step 5.3 -> Update the parameters
			SE3 refToNew_new = SE3::exp(inc.head<6>().cast<double>()) * refToNew_current;
			AffLight refToNew_aff_new = refToNew_aff_current;
			refToNew_aff_new.a += inc[6];
			refToNew_aff_new.b += inc[7];
			doStep(lvl, lambda, inc); // Update the idepth of keypoints

            // * Step 5.4 -> Criteria on whether to accept the optimization results based on updated and old energies
			...
			// * Step 5.5 -> (1) If accepted, then update the parameters (2) Otherwise, update lambda
			if(accept)
			{
				...
				applyStep(lvl);
				optReg(lvl); // update iR, from nearby iR
				lambda *= 0.5;
				fails=0;
            ...
            // Stop criteria (1) converge (2) > max number of iterations (3) fail twice
	...
	} // end for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
	// Sen -> Now we have gone through the optimization from the top lvl to the bottom

	thisToNext = refToNew_current;
	thisToNext_aff = refToNew_aff_current;

	// Sen -> (1) We now treat the estimates of the bottom lvl as the most accurate ones
    //        (2) Then we propagate iR and idepth of the bottom lvl to top 
    //        (3) The original iR of the upper lvls are discarded
		...
		bool plot)
{
	...
	for(int i=0;i<npts;i++)
	{
        ...
        VecNRf dp0; //typedef Eigen::Matrix<float,MAX_RES_PER_POINT,1> // MAX_RES_PER_POINT = 8 
        VecNRf dp1;
        VecNRf dp2;
        VecNRf dp3;
        VecNRf dp4;
		...
		// add into energy.
		E.updateSingle(energy); // Sen: energy is the sum of all pattern points
		point->isGood_new = true;
		point->energy_new[0] = energy;

		// update Hessian matrix.
		// Sen: _mm_load_ps(float *p): loads four SP FP values. The address must be 16-byte-aligned
		//		_mm_load_ps loads 4 single precision floating point values
		//		_mm_load_pd loads 2 double precision floating point values
		// Sen: therefore, i+=4 here, means we read 4 float each time, i.e. p[0], p[1], p[2], p[3]	
		//		each float is 4-byte, the result of _mm_load_ps -> 4x4=16 bytes
		for(int i=0;i+3<patternNum;i+=4)
			// Sen: updateSSE: save Hessian matrix as J^T x J
			acc9.updateSSE(
		...
		// Sen: what is range of i here? 
		// 		what is the purpose? to add every other points in to the first pattern point position within each 4 pattern point block?
		//	-> Figure out! The last for loop stop at i + 3 < patternNum, it may happens that there are some points left out by last for loop
		//	-> ((patternNum>>2)<<2) Take the modulo of 4
	...
	// Sen: H: typedef Eigen::Matrix<float,9,9> Mat99f;
	// Sen: topLeftCorner
...
// Update iR using nearest points (smooth)
void CoarseInitializer::optReg(int lvl)
{
	...
	for(int i=0;i<npts;i++)
	{
        ...
        // Use nearest 10 points with good quality
		for(int j=0;j<10;j++)
		{
        ...
        // Compute the new iR using the median of nearest points
		if(nnn > 2)
		{ 
			std::nth_element(idnn,idnn+nnn/2,idnn+nnn); 
			// Sen -> regWeight：set as 0.8
...
void CoarseInitializer::propagateUp(int srcLvl)
{
	...
	for(int i=0;i<nptst;i++)
	{
		...
		parent->iR=0;
		parent->iRSumNum=0;
        // Sen -> Does not matter if no values are presented in ptss's parents
        //     -> Will use neighbors for it in optReg
	}
	...
	{
		Pnt* parent = ptst+i;
        // Sen -> When a parent in nptss has multiple children, use Hessian to weight them
	...
	optReg(srcLvl+1); // Update iR adn idepth using neighbors
}
...
void CoarseInitializer::setFirst(	CalibHessian* HCalib, FrameHessian* newFrameHessian, float* disp_left)
{

	makeK(HCalib);
	firstFrame = newFrameHessian;

	// Sen -> We need to allocate memory here since disp_left will be delete in main_dso_pangolin.cpp immediately when processing next frame
	memcpy(firstDispLeft, disp_left, sizeof(float)*w[0]*h[0]);
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
			...
			// Sen -> Initialize idepth, iR to 1 for every lvl in the pyramid
			...
			{	
				...
				{
                    float depth = getDepthFromDisp(disp_left[x+y*wl]);
					pl[nl].idepth = 1.0f / depth;
					pl[nl].iR = 1.0f / depth;
				}
				...
				if (lvl!=0 && statusMapB[x+y*wl])
				{
					...
					// Sen -> average of four points
					// Sen -> pl, wl, hl, npts refer to points[lvl], w[lvl], h[lvl], ... in a certain lvl
                    int bidx = 2*x + 2*y*wlm1;
                    idepth_l[x + y*wl] = 0.25 * (idepth_lm[bidx] +
												idepth_lm[bidx+1] +
												idepth_lm[bidx+wlm1] +
...
void CoarseInitializer::doStep(int lvl, float lambda, Vec8f inc)
{
    ...
    // Sen -> Calculate the step and increment of idepth for each keypoint
	for(int i=0;i<npts;i++)
	{
