...
struct UnmappedFrame{
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW

	inline UnmappedFrame(FrameHessian* fh, float* disp_left) : unmappedFrame(fh), unmappedDispLeft(disp_left) {}

	FrameHessian* unmappedFrame;
	float* unmappedDispLeft;
};
...
public:
	...
	void addActiveFrame(ImageAndExposure* image, float* disp_left, float* disp_right, int id);
...
private:
	...
	CalibHessian Hcalib; // what does this used for?
	...
	void makeNewTraces(FrameHessian* newFrame, float* disp_left);
	void initializeFromInitializer(FrameHessian* newFrame, bool usePriorDisp);
 ...
 */
	...
	void makeKeyFrame( FrameHessian* fh, float* disp_left);
	void makeNonKeyFrame( FrameHessian* fh);
	void deliverTrackedFrame(FrameHessian* fh, bool needKF, float* disp_left);
	void mappingLoop();
	...
	std::deque<UnmappedFrame*> unmappedTrackedFrames;
