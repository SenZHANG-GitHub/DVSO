...
void FullSystem::flagFramesForMarginalization(FrameHessian* newFH)
{
    ...
    // Sen -> Discard all keyframes that turn bad after adding the last keyFrame fh
	for(int i=0;i<(int)frameHessians.size();i++)
	{
		...
		// Sen -> setting_minPointsRemaining = 0.05
...
//					fh->statistics_tracesCreatedForThisFrame, fh->statistics_pointsActivatedForThisFrame);
	...
	// marginalize one frame anyway to make space for fh
	if((int)frameHessians.size()-flagged >= setting_maxFrames)
	{
		...
		{
			// Sen -> setting_minFrameAge=1, only keep the current frame fh
...
void FullSystem::marginalizeFrame(FrameHessian* frame)
{
	...
	// Sen -> delete frame inside deleteOutOrder()
	//     -> delete frame::pointHessians in release()
