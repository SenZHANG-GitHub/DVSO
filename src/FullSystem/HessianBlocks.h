...
struct FrameFramePrecalc
{
	...
	// precalc values
	Mat33f PRE_RTll;    // rotation matrix R from host to target, after optimization
	Mat33f PRE_KRKiTll; // k*R*k_inv
	Mat33f PRE_RKiTll;  // R*k_inv
	Mat33f PRE_RTll_0;  // rotation matrix R from host to target, before optimization

	Vec2f PRE_aff_mode; 
	float PRE_b0_mode;  

	Vec3f PRE_tTll;   // translation from host to target, after optimization
	Vec3f PRE_KtTll;  // K*t
	Vec3f PRE_tTll_0; // htranslation from host to target, before optimization

	float distanceLL; // distance between two frames
...
struct FrameHessian
{
	...
	// Sen -> Vector3f: f means float, thus EIGEN_ALIGN16
	// Sen -> rather than disp_right -> Keep consistency with stereo_DSO_Sun's codes
	// Sen -> if this FrameHessian is frame_right -> only use frameID and dDisp
    //* Disparity gradients [0]:disparity value  [1]: gradient along x-axis  [2]: gradient along y-axis
	Eigen::Vector3f* dDisp; // gradients of disp_right

	// Sen -> if isRightFH is true: dDisp will have values, while dI, dIp, absSquaredGrad, frame_right are null
	//     -> if isRightFH is false: dI, dIp, absSquaredGrad, frame_right will have values, while dDisp is null
	bool isRightFH = false;
	...
	int frameID;			     // (FrameShell) incremental ID for keyframes only!
	static int instanceCounter;  
	int idx;				     // (FrameHessian)
	...
	inline ~FrameHessian()
	{
		// Sen -> Important: Need to judge whether dIp absSquaredGrad, and dDisp is 0!!!
		//			-> Now frame_left don't have dDisp and frame_right don't have dIp, absSquaredGrad!!!
		...
		if (isRightFH)
		{
			// Sen -> release the memory for dDisp
        	delete[] dDisp;
		}
    ...
    // Sen -> get the gradients of disp
    void makeDisps(float* disp, bool is_warped);
