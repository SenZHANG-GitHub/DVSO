...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians, bool isFirst)
{
	...
	if (isFirst)
	{
		// Sen -> For firstFrame fh, pointHessians have already been activated by the points in [firstFrame lvl=0] in initializeFromInitializerCNN()
		// Sen -> CoarseTracker->idepth corresponds to lastFrame, and obtained from ph->idepth/efPoint->HdiF 
		for(PointHessian* ph : lastRef->pointHessians)
		{
			// Sen -> For firstFrame, we have not optimized in the back-end yet
			int u = ph->u + 0.5f;
			int v = ph->v + 0.5f;
			float new_idepth = ph->idepth;

			// Sen -> initialization of ph->efPoint->HdiF does not matter since we will rescale to 1
	...
	for(int lvl=1; lvl<pyrLevelsUsed; lvl++)
	{
		...
		for(int y=0;y<hl;y++)
			for(int x=0;x<wl;x++)
			{
				...
				// Sen -> a point in last lvl correponds to 4 points in current lvl
...
{
	// Sen -> Calculate Jacobian and Hessian for photometric error
	acc.initialize();
...
Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
{
	...
	// Sen -> pc_* are assigned values in CoarseTracker::makeCoarseDepthL0()
	int nl = pc_n[lvl];
	float* lpc_u = pc_u[lvl];
	float* lpc_v = pc_v[lvl];
	float* lpc_idepth = pc_idepth[lvl];
	...
	for(int i=0;i<nl;i++)
	{
		...
		if(lvl==0 && i%32==0)
		{
			...
			// Sen -> Used to calculate the position change norm of optical flow: ||p-p'||
			// Sen -> shiftT:  optical flow for translation only
			// Sen -> shiftRT: optical flow for both translation and rotation
	...
	// Sen -> rs[0]: total energy or loss residuals
	rs[0] = E;
	rs[1] = numTermsInE;

	// Sen -> rs[2]: mean sum squared optical flow for trans only
	rs[2] = sumSquaredShiftT/(sumSquaredShiftNum+0.1);

	rs[3] = 0;

	// Sen -> rs[4]: mean sum squared optical flow for both trans and rot
	rs[4] = sumSquaredShiftRT/(sumSquaredShiftNum+0.1);

	// Sen -> numSaturated: number of res that > cutoffTH
...
// Track new frames and optimize for pose and photometric parameters
		...
		bool isFirst)
{
    ...
    // Sen -> Use the idepth of points projected to current frame to calculate the idepth for each pyramid lvl
	makeCoarseDepthL0(frameHessians, isFirst);
...
// Sen -> Use direct method，and the corresponding depths in the last keyframe (refFrame)
// Sen -> trackNewCoarse() for the frond-end tracking
		...
		IOWrap::Output3DWrapper* wrap)
{
	...
	// Sen -> lastResiduals: Vec5: 5 layers for image pyramid
	lastResiduals.setConstant(NAN);

	// Sen -> lastFlowIndicators: Vec3: rs[2], rs[3], rs[4] for layer 0
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
		...
		// Sen -> setting_coarseCutoffTH = 20
		// Sen -> IN calcRes(): Save the warped information into buf_warped_* to facilitate the calculation of calcGSSSE
		// Sen -> rs[0]=E,rs[1]=numTermsInE,rs[5]=numSaturated/numTermsInE
		...
		for(int iteration=0; iteration < maxIterations[lvl]; iteration++)
		{
			...
			Vec8 incScaled = inc;
			// Sen -> vector.segment<n>(i): Block containing n elements, starting at position i
		...
		// Sen -> vector.segment<n>(i): Block containing n elements, starting at position i
		// Sen -> lastFlowIndicators: rs[2]=sumSquaredShiftT/sumSquaredShiftNum, rs[3]=0, rs[4]=sumSquaredShiftRT/sumSquaredShiftNum
		...
		FrameHessian* frame)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		for(PointHessian* ph : fh->pointHessians)
		{
			...
			Vec3f ptp = KRKi * Vec3f(ph->u, ph->v, 1) + Kt*ph->idepth_scaled; // project to current frame
