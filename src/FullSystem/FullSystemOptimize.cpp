...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	for(int k=min;k<max;k++)
	{
		...
		if(fixLinearization)
		{
			...
			{
				// // SenFlagVS
				if(r->isNew && r->virtualStereoResFlag == false)
				// if(r->isNew)
				{
...
void FullSystem::setNewFrameEnergyTH()
{
	...
	for(PointFrameResidual* r : activeResiduals)
		...
		{
			// // SenFlagVS
			// if (r->virtualStereoResFlag == false)
			{
	...
	// printf("newFrame->frameEnergyTH: %.2f\n", newFrame->frameEnergyTH);
...
// Gauss-Newton optimization for current keyframe
float FullSystem::optimize(int mnumOptIts)
{
	...
	activeResiduals.clear(); //std::vector<PointFrameResidual*> 
	int numPoints = 0;
	int numLRes = 0;
	for(FrameHessian* fh : frameHessians)
		for(PointHessian* ph : fh->pointHessians)
		{
			// Sen -> ph->host / target frames
	...
	for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
		...
		// printf("============================================\n");
		// printf("newEnergy[1]: %.2f, newEnergyL: %.2f, newEnergyM: %.2f\n", newEnergy[1], newEnergyL, newEnergyM);
		// printf("lastEnergy[1]: %.2f, lastEnergyL: %.2f, lastEnergyM: %.2f\n", lastEnergy[1], lastEnergyL, lastEnergyM);
