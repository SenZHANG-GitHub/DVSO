...
void PhotometricUndistorter::processFrame(T* image_in, float exposure_time, float factor)
{
	...
	else
	{
		for(int i=0; i<wh;i++)
		{
			// Sen -> T* image_in -> T must be int type
			data[i] = G[image_in[i]];
		}
...
// Sen -> Wrapper for depth reader since output is no longer ImageAndExposure
DepthWrapper::DepthWrapper(int w_, int h_)
{
	w = w_;
	h = h_;
	output = new ImageAndDepth(w,h);
	valid = true;
}
DepthWrapper::~DepthWrapper()
{
	if(output != 0) delete output;
}
...
template<typename T>
void DepthWrapper::processFrame(T* image_in, float factor)
{
	int wh=w*h;
    float* data = output->depth;
	assert(output->w == w && output->h == h);
	assert(data != 0);
...
template void DepthWrapper::processFrame<unsigned short>(unsigned short* image_in, float factor);
...
Undistort::~Undistort()
{
	...
	if(photometricUndist != 0) delete photometricUndist;
	if(depthWrapper != 0) delete depthWrapper;
}
...
void Undistort::loadDepthWrapper()
{
	depthWrapper = new DepthWrapper(getOriginalSize()[0], getOriginalSize()[1]);
}
...
ImageAndExposure* Undistort::undistort(const MinimalImage<T>* image_raw, float exposure, double timestamp, float factor) const
{
	...
	//image_raw->data will be processed and stored into photometricUndist->output->image
	...
	photometricUndist->output->copyMetaTo(*result); // copy exposure_time
	...
	if (!passthrough)
	{
		...
		{
			// get interp. values
			// Undistort is not about intensity but about distortion -> thus remapping values to another position is sufficient
			// remapX/Y tells that which position should this pixel be located in the raw_image
			float xx = remapX[idx];
			float yy = remapY[idx];
...
template<typename T>
ImageAndDepth* Undistort::undistortDepth(const MinimalImage<T>* image_raw, double timestamp, float factor) const
{
    // Sen -> Current Depth reader can only deal with wOrg == w, hOrg == h
	if(image_raw->w != wOrg || image_raw->h != hOrg)
	{
		printf("Undistort::undistortDepth: wrong image size (%d %d instead of %d %d) \n", image_raw->w, image_raw->h, w, h);
		exit(1);
	}

	// // Sen => We do not do photometric calib for depths
	// // image_raw->data will be processed and stored into photometricUndist->output->image
	depthWrapper->processFrame<T>(image_raw->data, factor);

    // Sen -> results will be returned, not depthWrapper. Thus every time we have a new ImageAndDepth
	ImageAndDepth* result = new ImageAndDepth(w, h, timestamp);

	if (!passthrough)
	{
		float* out_data = result->depth;
		float* in_data = depthWrapper->output->depth;


		// Sen => Do we need noiseMap for depth?
		float* noiseMapX=0;
		float* noiseMapY=0;
		if(benchmark_varNoise>0)
		{
	...
	{
		memcpy(result->depth, depthWrapper->output->depth, sizeof(float)*w*h);
	}

	applyBlurNoise(result->depth);

	return result;
	
}
template ImageAndDepth* Undistort::undistortDepth<unsigned short>(const MinimalImage<unsigned short>* image_raw, double timestamp, float factor) const;
...
void Undistort::readFromFile(const char* configFileName, int nPars, std::string prefix)
{
	...
	{
		printf("\nError: Currently we only support relative calibration file format!\n");
		exit(1);
	}
	...
	//　l5: the fifth line that stores the baseline
    if(std::sscanf(l5.c_str(), "%lf", &baseline) == 1)
    {
        printf("Baseline: %lf \n", baseline);
		baselineXFocal = baseline * focal_x;
    }
    else
    {
	...
	// If the remapped X/Y coord is outside range -> set to -1 
	for(int y=0;y<h;y++)
		for(int x=0;x<w;x++)
		{
