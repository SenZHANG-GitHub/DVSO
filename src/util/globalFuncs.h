...
// Sen -> tranform monodepth predicted disparity to depth
inline float getDepthFromDisp(float disp)
{
    // baseline * focal_length / disp // for 00: 0.3128
    // float depth = 0.3128f / (disp + 0.00001f);

	//  Sen -> focal_length: normalized fx!
	// float depth = 0.3114431f / (disp + 0.00001f);
	float depth = (float)baselineXFocal / (disp + 0.00001f);

	// Sen -> Since the max GT of kitti depth is 80m 
