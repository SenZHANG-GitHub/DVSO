...
public:
	EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
	DepthWrapper(int w_, int h_);
	~DepthWrapper();

	// output will be written in [output].
	// Sen => factor should be 1./256. to recover real-scaled depth
	template<typename T> void processFrame(T* image_in, float factor);
	// void unMapFloatImage(float* image);

	ImageAndDepth* output;
...
public:
	...
	template<typename T>
	ImageAndDepth* undistortDepth(const MinimalImage<T>* image_raw, double timestamp=0, float factor=1) const;
