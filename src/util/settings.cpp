...
float w_energyStereo = -1;
float benchmark_initializerSlackFactor = 0;  // Sen -> for original DSO: 1
// float benchmark_initializerSlackFactor = 3;

bool useVirtualStereo = true; // (1) false: do not use virtual stereo residue; (2) true: use virtual stereo residue
bool judgeSqrtHW = true;
bool checkWarpValid = true; // whether check a warped right disparity pixel is valid for adding a residual
bool maskWarpGrad = true; // whether mask out gradients of invalid warped disparity pixels

int wStereoPosFlag = 1; // 1: before sqrtf(hw) (Before), 2: after sqrtf(hw) (After)
int wCorrectedFlag = 1; // 1: sqrtf(hw) (Ori), 2: sqrtf(hw*(2-hw)) (Corr)
int wGradFlag = 1; // 1: hitColor_left[1]/[2] (Hit), 2: d_I_x/y (Grad)

float scaleEnergyLeftTHR = 1.0;
float scaleWJI2SumTHR = 1.0;
...
bool setting_onlyLogKFPoses = false; // Now we output all poses
bool setting_logID = true; // Now we output id rather than timestamp
