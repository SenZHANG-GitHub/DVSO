...
public:
  ...
  inline void initialize()
  {
	H.setZero();
	b.setZero();
	// Sen: (1+2+..+9) = 45 since Hessian is symmetric
  ...
  {
	  // Sen:
	  //	! each J0-J8 is the result from _mm_load_ps(float *p) 
	  //	! which contains 4 elements of 4 byte values (corresponds to 4 pattern points)
	  //	! __m128 -> 128 bits -> 16 = 4x4 bytes
	  //	! _mm_mul_ps(_m128, _m128) takes two vectors of 4 floats or 4 bytes, and then multiply "one to one" the two vectors
	  //		which means we save all pattern points separately 
	//			not reallt all pattern points, since each byte memory space will save thge sum of two pattern points
	  // 	! by using _mm_* series operations, we ensure the alignment in memory and code efficiency 
	  //	! we only need to save upper-right triangle since it is Hessian is a symmetric matrix
  ...
  {
	  float* pt=SSEData+off;
	  // Sen: We only update the first float of each 4 float block 
	  // 	  (or the first pattern points within each 4 pattern point block)
	  *pt += J0*J0; pt+=4;
	  *pt += J1*J0; pt+=4;
	  *pt += J2*J0; pt+=4;
	  *pt += J3*J0; pt+=4;
