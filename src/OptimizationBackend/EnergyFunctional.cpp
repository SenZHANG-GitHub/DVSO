...
bool EFAdjointsValid = false; // Sen -> set as true in setAdjointsF()
...
EFResidual* EnergyFunctional::insertResidual(PointFrameResidual* r)
{
	...
	// // SenFlagVS
	if(efr->data->virtualStereoResFlag == false)
	{
...
EFFrame* EnergyFunctional::insertFrame(FrameHessian* fh, CalibHessian* Hcalib)
{
	...
	// Sen -> Only left image fh->efFrame will be added into FullSystem::frames
	...
	//stereo
	// Sen -> May have bug! Since frame_right has nothing but disp and dI_disp
	...
	// Sen -> set Mat88[nFrames*nFrames] adHost, adTarget
	setAdjointsF(Hcalib); 
	makeIDX(); 

	// Sen -> here we consider the covisibility between frames
	for(EFFrame* fh2 : frames)
	{
        // Sen -> connectivityMap: residue count between two frames
        // Sen -> connectivityMap: std::map -> is a hashing table!
...
void EnergyFunctional::dropResidual(EFResidual* r)
{
    ...
    // // SenFlagVS
	// Sen -> connectivityMap save the temporal covisibility between non-stereo images
    //     -> stereo images are regarded as covisible by default already
    if(r->data->virtualStereoResFlag == false)
    {
	...
	for(EFFrame* f : frames)
		for(EFPoint* p : f->points)
		{
			...
			for(EFResidual* r : p->residualsAll)
			{
				...
				// Sen -> For stereo res: host and target are of the same idx
                // // SenFlagVS
                if(useVirtualStereo && r->data->virtualStereoResFlag == true){
